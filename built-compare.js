...
orion.LineNumberCompareRuler = (function() {
	...
	LineNumberCompareRuler.prototype.onDblClick = function(e) {
		var obj = {
			type: "dbclickline",
			line: e+1
		}
...
define("orion/editor/textView", [ //$NON-NLS-0$
	...
	TextView.prototype = /** @lends orion.editor.TextView.prototype */ {
		...
		*/
		...
		//Che-Code review plugin
		//catches the selection and define the entrypoint, endpoint, lineindex, optional: text?
		_catchSelection: function (){
				var selection = this._getSelection();
				if (selection) {
					var selectiontext = window.getSelection();
					//var show = document.getElementById("showselection").innerHTML;
					//sandbox: Razne moznosti dela z modelom

					var model = this._model;
					var end = selection.end;
					var start = selection.start;
					var lineIndexend = model.getLineAtOffset(selection.end)+1;
					var lineIndexstart = model.getLineAtOffset(selection.start)+1;
					/*model.getLineEnd(lineIndex);
					var line = this._getLine(lineIndex);
					selection.extend(line.getNextOffset(selection.getCaret(), {unit:"character", count:1})); //$NON-NLS-0$*/

					document.getElementById("showselection").innerHTML = "-->first line "+lineIndexstart+"-->last line: "+ lineIndexend+" -->SELECTION--> "+selectiontext ;
					var obj = {
						type: "commentselection",
						firstline:lineIndexstart,
						lastline:lineIndexend
					}
					parent.postMessage(obj,"*");
		...
		_hookEvents: function() {
			...
			if (util.isIOS || util.isAndroid) {
				...
				//handlers.push({target: clientDiv, type: "mouseup", handler: function(e) { return self._handleMouseDown(e ? e : window.event);}}); //$NON-NLS-0$
